...
class BasePDF(FPDF):
    ...
    def header_brand(self):
        ...
        try:
            self.image("Brasao_versaooficial.jpg", x=95, y=4, w=20)
        except Exception:
            # Caso a imagem não seja encontrada, o PDF continua sem erro
            pass
            
        self.ln(15) # Pula espaço para não escrever em cima da imagem
...
# DADOS (GSHEETS)
# =========================================================
conn = st.connection("gsheets", type=GSheetsConnection) df = conn.read() df.columns = [str(c).strip() for c in df.columns] nomes = [] if "Nome do Estudante" in df.columns: nomes = df["Nome do Estudante"].dropna().unique().tolist()
